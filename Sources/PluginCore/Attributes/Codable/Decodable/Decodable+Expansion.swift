...
extension ConformDecodable: MemberMacro, ExtensionMacro {
    /// Expand to produce extensions with `Decodable` implementation
    /// members for attached `class`.
    ///
    /// Conformance for `Decodable` is generated regardless
    /// of whether class already conforms to it. Class or its super class
    /// shouldn't conform to `Decodable`
    ///
    ...
    ///   conformance with `init(from:)` implementation depending on already
    ...
    /// `Decodable` conformance implementation is skipped.
    ...
    ///
    /// Depending on whether attached type already conforms to `Decodable`,
    /// extension for `Decodable` conformance implementation is skipped.
    /// Entire macro expansion is skipped if attached type already
    /// conforms to `Decodable`.
    ///
    /// The `AttributeExpander` instance provides declarations based on
